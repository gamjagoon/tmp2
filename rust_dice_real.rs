...
const DICE_MODE_NOT_INIT:u8 = 0u8;
const DICE_MODE_NORMAL:u8 = 1u8;
const DICE_MODE_DEBUG:u8= 2u8;
const DICE_MODE_MAINTENCANCE:u8=3u8;

const DICE_CONFIG_INLINE: u8 = 0u8;
const DICE_CONFIG_DESCRIPTOR: u8 = 1u8;
...
fn encode_cwt(
    ...
    authority_id_hex: &Vec<u8>,
    subject_id_hex: &Vec<u8>,
    ...
    match input_values.config_type {
        DICE_CONFIG_DESCRIPTOR =>map_pairs += 2,
        DICE_CONFIG_INLINE =>map_pairs += 1,
        _=> (),
    }
    ...
    let authority_id_str = std::str::from_utf8(&authority_id_hex).unwrap();
    let subject_id_str = std::str::from_utf8(&subject_id_hex).unwrap();

    enc.map(map_pairs).unwrap()
        .i64(CWT_ISSUER_LABEL).unwrap()
        .str(&authority_id_str).unwrap()
        .i64(CWT_SUBJECT_LABEL).unwrap()
        .str(&subject_id_str).unwrap()
    ...
    enc.i64(MODE_LABEL).unwrap()
        .bytes(&[input_values.mode]).unwrap()
...
fn dice_generate_certificate(
    ...
    let subject_id_hex = hex_encode(subject_id.as_slice());
    let authority_id_hex = hex_encode(authority_id.as_slice());
    ...
    println!("[cwt payload], size = {}",511);
    for i in &payload[..511] {
        print!("{:02X}", i);
    }
    println!();
...
mod tests {
    ...
    fn kat_cose_zero_input() {
        ...
        certificate_buffer.resize(cert_len,0);
        println!();
        debug_assert_eq!(cert_len, expected_output.len(), "Does not match expected len");
        debug_assert_eq!(certificate_buffer.as_slice(), expected_output, "Does not match expected value");
